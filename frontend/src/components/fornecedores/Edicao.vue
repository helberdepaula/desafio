...
import type { CreateFornecedorDto, Fornecedor, UpdateFornecedorDto } from '@/types/fornecedores-type';
...
const { updateFornecedor, fornecedor, errorForn, find } = useFornecedores()
...
const { handleSubmit, handleReset, setFieldValue, setValues } = useForm<CreateFornecedorDto>({
...
const submitForm = handleSubmit(async (data: UpdateFornecedorDto) => {
    const result = await updateFornecedor(Number(id), data)
...
watch(fornecedor, async (data: Fornecedor | null) => {
    if (data) {

        const result = await fetchMunicipio(data.endereco.municipio.estado.id)
        const municipio = result?.find(item => item.id == Number(data.endereco.municipio.id))
        if (municipio) {
            setFieldValue('municipio_id', municipio.id)
        }
        setFieldValue('estado_id', data.endereco.municipio.estado.id)
        setValues({
            nome: data.nome,
            cnpj: data.cnpj,
